...
//   function iter(data, level) {
//     let result = [];
//     const indent = ' '.repeat(level * 2); // Увеличил отступ

//     for (const key in data) {
//       result.push(`${indent}${key} 1`);
//       if (typeof data[key] === 'object' && data[key] !== null) {
//         // Если это объект, мы не добавляем его значение
//         result.push(iter(data[key], level + 1));
//       } else {
//         result.push(`${indent}${key}:${data[key]}`);
//       }
  ...
  function iter(obj,level) {
    ...
    for (const prop in obj) {
      if(obj.hasOwnProperty(prop)) {
        const value = typeof obj[prop] === 'object' && obj[prop] !== null
          ? JSON.stringify(obj[prop], null, 2): obj[prop];
